...
const InputLighthouse = memo(
  ({ fileSpec, onFileUpload, onFileUploadError, onSubmit, onReply }: Props) => {
    ...
    const submit = useCallback(() => {
      alert('정말로 전송하시겠습니까? (Lighthouse❗️)');

      if (value === '' || disabled) {
        alert('메시지를 입력해주세요. (Lighthouse❗️)');
        return;
      }
      if (askUser) {
        onReply(value);
    ...
    return (
      ...
      >
        ...
        <TextField
           ...
           */
          placeholder={t(
            'components.organisms.chat.inputBox.input_lighthouse.placeholder'
          )}
          disabled={disabled}
...
export default InputLighthouse;
